    ...
    AND (
      ...
      OR EXISTS (
        ...
        WHERE ph.post_id = p.id
          AND lower(h.tag) = v_hashtag_norm
      )
    )
    AND (
      CASE
...
COMMENT ON FUNCTION public.get_universal_feed(uuid, text, uuid, uuid, uuid, int, int, text, text) IS
'Returns paginated universal feed for a viewer. Optional p_hashtag filters posts that include the given tag (case-insensitive, with or without leading #). Security: search_path set to public.';