...
CREATE INDEX IF NOT EXISTS idx_activity_events_event_type ON public.activity_events(event_type, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_activity_events_user_created ON public.activity_events(user_id, created_at DESC, event_type);
ALTER TABLE public.activity_events ENABLE ROW LEVEL SECURITY;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='activity_events' AND policyname='Users read their own activity') THEN
    CREATE POLICY "Users read their own activity" ON public.activity_events FOR SELECT USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='activity_events' AND policyname='Users insert their own activity') THEN
    CREATE POLICY "Users insert their own activity" ON public.activity_events FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='activity_events' AND policyname='Service role manages activity') THEN
    CREATE POLICY "Service role manages activity" ON public.activity_events FOR ALL USING (auth.role() = 'service_role') WITH CHECK (auth.role() = 'service_role');
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'brief_interaction_type') THEN
    CREATE TYPE brief_interaction_type AS ENUM ('viewed','clicked','dismissed','not_interested','saved','why_this_opened');
  END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_brief_interactions_user_card ON public.dia_brief_interactions(user_id, card_id);
CREATE INDEX IF NOT EXISTS idx_brief_interactions_type ON public.dia_brief_interactions(user_id, interaction_type, created_at DESC);
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='trend_follows' AND policyname='Users read their own follows') THEN
    CREATE POLICY "Users read their own follows" ON public.trend_follows FOR SELECT USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='trend_follows' AND policyname='Users create their own follows') THEN
    CREATE POLICY "Users create their own follows" ON public.trend_follows FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='trend_follows' AND policyname='Users delete their own follows') THEN
    CREATE POLICY "Users delete their own follows" ON public.trend_follows FOR DELETE USING (auth.uid() = user_id);
  END IF;
END $$;
...
CREATE UNIQUE INDEX idx_pulse_metrics_unique ON public.pulse_metrics_daily(hour_bucket, c_module);
CREATE INDEX idx_pulse_metrics_recent ON public.pulse_metrics_daily(hour_bucket DESC, c_module);
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron') THEN
    ...
    END IF;
    PERFORM cron.schedule('refresh-pulse-metrics', '*/5 * * * *',
...
RETURNS TABLE (c_module TEXT, event_count BIGINT, unique_users BIGINT, delta_vs_prior_period BIGINT)
...
BEGIN
  ...
  IF p_scope = 'user' THEN
    ...
    WITH event_to_c AS (
      ...
      WHERE ae.user_id = v_user_id AND ae.created_at > v_now - (v_interval * 2)
    ),
    current_period AS (
      SELECT etc.c_mod, COUNT(*)::BIGINT AS events FROM event_to_c etc
      WHERE etc.created_at > v_now - v_interval AND etc.c_mod <> 'other'
      GROUP BY etc.c_mod
    ),
    prior_period AS (
      SELECT etc.c_mod, COUNT(*)::BIGINT AS events FROM event_to_c etc
      WHERE etc.created_at > v_now - (v_interval * 2) AND etc.created_at <= v_now - v_interval AND etc.c_mod <> 'other'
      GROUP BY etc.c_mod
    ),
    all_cs AS (SELECT unnest(ARRAY['connect','convene','collaborate','contribute','convey']) AS c_mod)
    SELECT ac.c_mod, COALESCE(cp.events, 0),
      CASE WHEN COALESCE(cp.events, 0) > 0 THEN 1::BIGINT ELSE 0::BIGINT END,
      COALESCE(cp.events, 0) - COALESCE(pp.events, 0)
    FROM all_cs ac
  ...
  ELSE
    ...
    WITH current_period AS (
      SELECT pm.c_module AS c_mod, SUM(pm.event_count)::BIGINT AS events, SUM(pm.unique_users)::BIGINT AS users
      FROM public.pulse_metrics_daily pm
      WHERE pm.hour_bucket > v_now - v_interval AND pm.c_module <> 'other'
      GROUP BY pm.c_module
    ),
    prior_period AS (
      SELECT pm.c_module AS c_mod, SUM(pm.event_count)::BIGINT AS events
      FROM public.pulse_metrics_daily pm
      WHERE pm.hour_bucket > v_now - (v_interval * 2) AND pm.hour_bucket <= v_now - v_interval AND pm.c_module <> 'other'
      GROUP BY pm.c_module
    ),
    all_cs AS (SELECT unnest(ARRAY['connect','convene','collaborate','contribute','convey']) AS c_mod)
    SELECT ac.c_mod, COALESCE(cp.events, 0), COALESCE(cp.users, 0), COALESCE(cp.events, 0) - COALESCE(pp.events, 0)
    FROM all_cs ac
    LEFT JOIN current_period cp ON cp.c_mod = ac.c_mod
    LEFT JOIN prior_period pp ON pp.c_mod = ac.c_mod;
  END IF;
...
RETURNS TABLE (event_type TEXT, event_count BIGINT, display_label TEXT)
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
DECLARE
  v_interval INTERVAL;
...
RETURNS TABLE (
  id UUID, "position" SMALLINT, c_module TEXT, signal_type TEXT, title TEXT, body TEXT,
  cta_label TEXT, cta_route TEXT, target_entity_type TEXT, target_entity_id UUID,
  reasoning TEXT, is_fallback BOOLEAN
)
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
DECLARE
...
BEGIN
  ...
  END IF;
  RETURN QUERY
  SELECT bc.id, bc."position", bc.c_module, bc.signal_type, bc.title, bc.body,
    bc.cta_label, bc.cta_route, bc.target_entity_type, bc.target_entity_id,
    bc.reasoning, bc.is_fallback
  FROM public.dia_brief_cards bc
  WHERE bc.user_id = v_user_id AND bc.expires_at > now()
    AND NOT EXISTS (
      SELECT 1 FROM public.dia_brief_interactions bi
      WHERE bi.card_id = bc.id AND bi.interaction_type IN ('dismissed','not_interested')
    )
...
BEGIN
  ...
  IF p_interaction_type IN ('dismissed','not_interested') THEN
    UPDATE public.dia_brief_cards SET expires_at = now()
...
BEGIN
  ...
  RETURN QUERY
  SELECT ph.hashtag,
    COUNT(DISTINCT ph.post_id)::BIGINT,
    COUNT(DISTINCT p.author_id)::BIGINT,
    EXISTS(SELECT 1 FROM public.trend_follows tf WHERE tf.user_id = v_user_id AND tf.hashtag = ph.hashtag)
  FROM public.post_hashtags ph
  JOIN public.posts p ON p.id = ph.post_id
  WHERE p.created_at > now() - v_interval AND p.visibility = 'public'
  GROUP BY ph.hashtag
...
BEGIN
  IF v_user_id IS NULL THEN
    RAISE EXCEPTION 'Authentication required';
  END IF;
  SELECT EXISTS(SELECT 1 FROM public.trend_follows WHERE user_id = v_user_id AND hashtag = p_hashtag) INTO v_exists;
  IF v_exists THEN
    DELETE FROM public.trend_follows WHERE user_id = v_user_id AND hashtag = p_hashtag;
    RETURN FALSE;
  ELSE
    INSERT INTO public.trend_follows (user_id, hashtag) VALUES (v_user_id, p_hashtag);
    RETURN TRUE;
  END IF;
END;
$$;
...
CREATE TRIGGER trg_broadcast_activity_event
  ...
  FOR EACH ROW EXECUTE FUNCTION public.broadcast_activity_event();