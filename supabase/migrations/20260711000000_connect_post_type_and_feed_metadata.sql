-- Connect pipe (Phase C): the fifth card writes posts(post_type='connect').
--
-- 1. posts_post_type_check did not allow 'connect' — without this, every
--    Connect post fails the check constraint at insert time.
ALTER TABLE public.posts DROP CONSTRAINT IF EXISTS posts_post_type_check;
ALTER TABLE public.posts ADD CONSTRAINT posts_post_type_check
  CHECK (post_type IN ('post', 'status', 'story', 'event', 'space', 'need', 'reshare', 'community_post', 'connect'));

-- 2. Surface posts.metadata in get_universal_feed so Connect facets
--    (intent / direction / sector / where) survive into the feed row.
--    Adding a return column changes the function signature's OUT list,
--    so DROP + CREATE (body otherwise identical to the prior definition).
...
CREATE FUNCTION public.get_universal_feed(
  ...
  p_ranking_mode text DEFAULT 'latest'::text,
  p_hashtag text DEFAULT NULL::text
)
RETURNS TABLE(
  ...
  space_id uuid, event_id uuid, created_at timestamp with time zone,
  updated_at timestamp with time zone, likes_count bigint, comments_count bigint,
  user_has_liked boolean, user_has_bookmarked boolean, original_post_id uuid,
  original_author_id uuid, original_author_username text,
  original_author_full_name text, original_author_avatar_url text,
  original_author_headline text, original_content text, original_image_url text,
  original_created_at timestamp with time zone, slug text, metadata jsonb
)
LANGUAGE plpgsql
SECURITY DEFINER
