...
export const searchContent = async (query: string, filters?: SearchFilters): Promise<SearchResult[]> => {
  ...
  if (!filters?.types.length || filters.types.includes('event')) {
    const { data: events, error: eventsError } = await supabase
      .from('events')
      .select('id, title, description, location_name, location_city, start_time, time_confirmed, date_confirmed, cover_image_url, created_at, event_type, is_cancelled')
      .eq('status', 'published')
    ...
    if (!eventsError && events) {
      results.push(...events.map(event => ({
        ...
        metadata: {
          ...
          // An unannounced placeholder date must never print downstream.
          date_time: event.date_confirmed === false ? null : event.start_time,
